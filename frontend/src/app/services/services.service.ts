...
export class ServicesService {
  private baseUrl = 'http://127.0.0.1:8000'; //Rest Api Endpoint
  ...
  getHotels(): Observable<any[]> {
    return this.http.get<any[]>(`${this.baseUrl}/api/hotels`);
  }

  // Define the writeData method to send data to the backend server
  writeData(formData: any): Observable<any> {
    return this.http.post<any>(`${this.baseUrl}/api/bookings`, formData);
  }

  bookingData(user_id: number, hotel_id: number, customer_name: string, no_people: number, start_date: string, end_date: number, rooms_book: number): Observable<any> {
    return this.http.post<any>(`${this.baseUrl}/api/bookings`, { user_id, hotel_id, customer_name, no_people, start_date, end_date, rooms_book });
  }
