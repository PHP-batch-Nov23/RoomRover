...
">
    ...
    <div class="items-center" id="mydata">
        ...
        <form action="{{ route('bookings.search') }}" method="GET" class="mb-3">
            <div class="input-group">
                <input type="text" name="search" class="form-control" placeholder="Search by Agent" value="{{ request('search') }}">
        ...
        <table class="table table-striped">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $booking->hotel ? $booking->hotel->name : 'Hotel not found' }}</td>
                    <td>{{ $booking->no_people }}</td>
                    <td>{{ $booking->rooms_book}}</td>
                    <td>{{ $booking->start_date}}</td>
                    <td>{{ $booking->end_date}}</td>
                    <td>
                        @if($booking->end_date < now()) Completed @else <form action="{{ route('booking.destroy',$booking->id )}}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger btn-danger reject-btn" style="
                                    height: 40px;
                                    width: 60px;
                                    padding: 2px;">
                                <i class="bi-person-x-fill" style="
                                    padding: 14px;
                                    margin: 0px;
                                    border-radius: 4px;
                                    font-size: 25px;"></i>
